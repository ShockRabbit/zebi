...
function install_process_pyenv
    ...
    set -l global (cat $config_path | jq -r ".pyenv | .global")
    set -l versions (cat $config_path | jq -r ".pyenv | .versions[].version")
    for v in $versions
        log "create pyenv python version $v"
        pyenv install $v || log_error "[pyenv] fail :: create pyenv python version $v"
        
        set -l names (cat $config_path | jq -r ".pyenv | .versions[] | select(.version==\"$v\") | .envs[].name")
        for n in $names
            ...
            set -l packages (cat $config_path | jq -r ".pyenv | .versions[] | select(.version==\"$v\") | .envs[] | select(.name==\"$n\") | .packages[]")
            for p in $packages
                log "pip install $p"
